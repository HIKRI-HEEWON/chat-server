...
public class StompWebSocketConfig implements WebSocketMessageBrokerConfigurer {
    ...
    public void configureMessageBroker(MessageBrokerRegistry registry) {
        // /publish/~ 형태로 메시지를 발행해야 함을 설정
        // /publish/~ 형태로 시작하는 url 패턴으로 메시지가 발행되면 @Controller 객체의 @MessageMapping 붙은 메서드를 호출하여 처리(라우팅)
        registry.setApplicationDestinationPrefixes("/publish");

        // /topic/~ 형태로 메시지를 수신(subscribe)해야 함을 설정
        // registry.enableStompBrokerRelay() << RabbitMQ 커스텀? 메서드 확인해보기 todo
